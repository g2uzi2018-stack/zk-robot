...
inline void validateTiming(const MoveJTimingOptions &timing)
{
    ...
    const long double range_seconds =
        std::chrono::duration<long double>(
            timing.maximum_duration - timing.minimum_duration).count();
    const long double resolution_seconds =
        std::chrono::duration<long double>(
            timing.search_resolution).count();
    ...
    const MoveJTimingOptions &timing)
{
    ...
    while (true)
    {
        ...
        {
            return detail::makePlan(start, goal, limits, candidate);
        }
...
template <std::size_t N>
PlannedJointMotion<N> planMoveJ(
    const std::array<double, N> &start_position,
    const std::array<double, N> &goal_position,
    const JointMotionLimits<N> &limits,
    const MotionDuration minimum_duration)
{
