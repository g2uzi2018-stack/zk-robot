    ...
    namespace detail
    {
        ...
        bool trajectorySatisfiesLimits(const JointBoundaryState<N> &start, const JointBoundaryState<N> &goal, const JointMotionLimits<N> &limits, const MotionDuration duration)
        {
            ...
            for (std::size_t joint = 0; joint < N; ++joint)
            {
                ...
                const auto coefficients = robot::motion::makeNormalizedQuinticCoefficients(start.position[joint], start.velocity[joint], start.acceleration[joint], goal.position[joint], goal.velocity[joint], goal.acceleration[joint], T);

                const Polynomial position(coefficients.begin(), coefficients.end());
                ...
                const ValueRange position_range = polynomialRange(position, 1.0);
                const ValueRange velocity_range = polynomialRange(velocity, inverse_T);
                const ValueRange acceleration_range = polynomialRange(acceleration, inverse_T2);
                const ValueRange jerk_range = polynomialRange(jerk, inverse_T3);

                const double position_tolerance = 1e-9 * std::max({1.0, std::abs(limits.min_position[joint]), std::abs(limits.max_position[joint])});
                // 和 joint limits 比较
