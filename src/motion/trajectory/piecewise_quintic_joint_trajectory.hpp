...
namespace robot::motion
{
    ...
    //     q0 → q1 → q2 → ... → qN
    //
    // 相邻两个 waypoint 之间建立一个 QuinticJointTrajectory。
    //
    // ------------------------------------------------------------
    //
    // 典型来源:
    ...
    class PiecewiseQuinticJointTrajectory final : public JointTrajectory<N>
    {
    public:
        ...
        //
        // 如果相邻 segment 在 waypoint 处使用相同的 velocity 和
        // acceleration，可以实现更好的连续性。
        struct Waypoint
        {
    ...
    public:
        explicit PiecewiseQuinticJointTrajectory(
            const std::vector<Waypoint> &waypoints)
        {
            ...
            if (waypoints.size() < 2)
            {
                throw std::invalid_argument(
                    "Piecewise quintic trajectory requires at least two waypoints");
            }

            // 为了让 sample(elapsed) 的时间定义简单明确，第一 waypoint 必须对应
            // trajectory 起始时刻。
            ...
            for (std::size_t i = 0; i + 1 < waypoints.size(); ++i)
            {
                ...
                typename QuinticJointTrajectory<N>::BoundaryState start_state{
                    start.position, start.velocity, start.acceleration};
                typename QuinticJointTrajectory<N>::BoundaryState goal_state{
                ...
                segments_.push_back(Segment{
                    ...
                    QuinticJointTrajectory<N>(
                        start_state, goal_state, segment_duration)});
            }
    ...
    public:
        Point sample(Duration elapsed) const override
        {
            ...
            {
                return segments_.back().trajectory.sample(
                    segments_.back().trajectory.duration());
            }

            // 第一版使用简单线性搜索。waypoint 数量通常不会非常大，而且代码最容易
            // 理解；以后如果 waypoint 数量很大，可以再改成 binary search。
            for (const auto &segment : segments_)
            {
                if (elapsed < segment.end_time)
                {
                    ...
                    // 整条 Piecewise trajectory 还没有结束。即使某个内部 segment
                    // 已经接近结束，finished 仍然应该表示整条 trajectory。
